	...
	req.on("end", function() {
		...
		xml2js.parseString(messageXml, function (error, result) {
			...
			switch(message.MsgType) {
				case "text":
					_replyTextMessage.call(self, message, function (replyMessageXml) {
...
_replyTextMessage = function (message, callback) {
	...
	for (i = 0; i < this.textRules.length; i++) {
		...
		if (nodeUtil.isRegExp(rule.pattern)) {
			...
			matchRule = utils.containsCaseInsensitive(messageContent, rule.pattern);
			...
			for (j = 0; j < rule.pattern.length; j++) {
				...
				if (nodeUtil.isRegExp(innerPattern)) {
					...
					matchRule = utils.containsCaseInsensitive(messageContent, innerPattern);
				}
...
WeChatMP = function (token, appId, appSecret) {
	this.appId = appId;
...
WeChatMP.prototype.run = function (port) {
	var self = this;
